...
const STORAGE_KEY = 'uau-sefaz-sync-empresas';
...
export const useConfigManager = () => {
  ...
  const addEmpresa = useCallback(async (newEmpresaData: Omit<Empresa, 'id'>) => {
    setLoading(true);
    try {
  ...
  const updateEmpresa = useCallback(async (id: string, updatedData: Partial<Omit<Empresa, 'id'>>) => {
    setLoading(true);
    try {
        await new Promise(res => setTimeout(res, 500)); // Simulate async
